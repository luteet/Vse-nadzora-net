...
let randKey, inputAlt, inputCode;
randKey = Math.floor(Math.random() * arrCode.length);
img.setAttribute('src', (imgSrc) ? `${imgSrc}${randKey}.jpg` : `img/captcha/${randKey}.jpg`);

input.setAttribute('alt', randKey);
...
form.addEventListener('submit', falseSubmit);
...
btn.addEventListener('click', function() {
    ...
    if(inputCode != arrCode[inputAlt]){
        ...
        message.textContent = 'Доступ не ограничивается'

        form.removeEventListener('submit', falseSubmit);
